...
| 同 EP shard 的 eFSDP/replicate sparse-grad 求和 | `HostEngramTable.reduce_sparse_gradient_across_replicas` | `engram_replica_grad_worker.py` 真实 4-rank Gloo mesh，验证同 shard replica sum / 不同 EP shard 隔离 / empty replica | 已覆盖 |
...
| sparse replica reduction | `wire_sparse_grad_replicas`, `reduce_sparse_gradient_across_replicas` | 4-rank Gloo | 已覆盖 | 修复子进程 cleanup（R8） |
| global clip / SparseAdam / checkpoint key | `HostSparseOptimizersContainer` | sparse norm、step、zero_grad、state dict suffix | 已覆盖语义 | R1 改成稳定 hook 后，测试应针对 hook 的实例作用域补一个“退出/另一个 Trainer 不受影响”断言 |
...
| sparse table replica over eFSDP | 4-rank, EP2 + DP shard4 | 2-rank case没有同 shard replica；必须验证 replica sum | `dsv41_engram_torch_ep2_fsdp4_resume` | 新增充分 | 保持 <=4 NPU |
...
| `tests/integration_tests/deepseek_v41.py` | 2P/4P Torch resume case 设计合理；AscendC suite 静态注册正确；metadata 与实际 golden env 有偏差，见 R5/R7。 |
...
| `tests/unit_tests/models/deepseek_v41/engram_replica_grad_worker.py` | 真 4-rank replica-grad worker，语义覆盖合理；launcher cleanup 见 R8。 |
...
| `tests/unit_tests/models/deepseek_v41/test_engram_replica_grad.py` | 真实 4-rank mesh 是必要覆盖；timeout cleanup 见 R8。 |
| `tests/unit_tests/models/deepseek_v41/test_engram_sparse_grad.py` | clip/replica/checkpoint suffix 语义覆盖充分；R1 重构后需把测试目标从 global patch 改为实例 hook。 |
...
| `torchtitan_npu/extensions/components/optimizer.py` | 主要语义实现可读，但 global clip monkey patch 是 blocker，见 R1；optimizer state materialization 需保留针对 fixed upstream v0.3.0 的原因说明。 |
...
| `torchtitan_npu/models/deepseek_v41/config_registry.py` | SparseAdam group + CLI 开关方向合理；新增 model-specific Trainer Config 与双状态见 R3。 |
...
| `torchtitan_npu/models/deepseek_v41/engram_host.py` | Host-authoritative table、sparse-grad、checkpoint shard 语义集中；global registration/R1、私有 mesh `_flatten`/R9 需处理。 |
...
| `torchtitan_npu/models/deepseek_v41/parallelize.py` | Host table 在 FSDP 外管理的需求合理；`FunctionType` adapter 不可接受，见 R2。 |
...
| 顺序 | 必须完成的修改 | 验证方式 |
| ---: | --- | --- |
| 1 | 删除 R1 的进程级 `clip_grad_norm_` monkey patch，改成显式、实例级、可上游的 grad-clip 扩展点 | UT 增加“构造/销毁 Engram trainer 不改变另一个普通 Trainer 的 clip callable/行为”；现有 sparse norm 数值 oracle 保留 |
| 2 | 删除 R2 的 `FunctionType(__code__/__globals__)` 适配，提供上游可接受的 `ignored_params`/filter hook | fixed v0.3.0 patch 或升级后的公开签名上直接断言 Host table weight 被 ignored，其余 FSDP 行为复用上游 helper |
