...
| Maintainer 总体结论 | **暂不建议按当前形态合入，Request changes。** SFT 产品语义本身成立，但当前实现同时引入第二套数据生命周期、第二个训练 shell 入口、SFT 专属 resize 分叉以及一套对官方 `encoding.py` 内部语义的第二份实现。应先显著删减/收敛架构，再补测试。 |
...
| R2 | **Blocker / 解耦** | `torchtitan_npu/models/deepseek_v4_1/vision_language_encoder.py:20-161`，尤其 `:47-61, 88-123`；`vision_language_dataset.py:179-219` | **动态加载任意版本的官方 `encoding.py`，但又在本仓复刻其关键消息处理语义：drop-thinking、tool merge/sort、逐 message render，并据此自行推导 assistant span。** `_drop_thinking_messages` 不是对官方函数逐行复制，而是精简重实现；问题仍然是仓内存在第二份同语义实现，依赖官方内部流程保持不变。 | PR 最后用 `rendered_prompt != prompt` 作为运行时一致性断言；一旦官方 encoding 升级，本仓不会向前兼容，而是训练直接崩。该风险已有可见证据：截至 2026-09-20，官方 V4.1 `encoding.py` 的 `_drop_thinking_messages` keep roles 已包含 `direct_search_results`，PR 的精简实现没有该角色。另一个职责重叠是图片标签：官方 `encode_messages` 本身通过 `process_image_messages` / `parse_tagged_text` 处理 `<image>path</image>` 并转为 content blocks，而 PR 的数据层 `split_text` 又提前解析同一种 tagged image。这里应区分两类语义：LLaVA 的 bare `<image>` + row-level `image/images` 映射是数据 schema adapter 的真实新增职责；已带路径的 `<image>path</image>` 则不宜在数据层和官方 encoding 两层重复解析。官方代码见 [DeepSeek-V4.1 encoding.py](https://huggingface.co/deepseek-ai/DeepSeek-V4.1-Flash/blob/main/encoding/encoding.py)。 | **不要长期维护第二份 encoding 语义实现。** 首选推动/使用官方 encoding 暴露稳定的 rendered assistant spans / supervision mask 接口，本仓仅做薄 adapter；短期若必须兼容当前资产，也应把依赖缩到最小、显式校验所需 API/资产 revision，并避免自行复刻 `_drop_thinking_messages` 等内部语义。图片 schema 也应只在一层负责：保留 bare `<image>` + 外部 image list 的必要归一化，已由官方支持的 tagged path 直接交给官方处理。不能把“最终字符串相等，否则 RuntimeError”当成解耦方案。 |
...
| R4 | **High / 单一协议** | `vision_language_dataset.py:35-46`；对照 `torchtitan_npu/models/deepseek_v4_1/vision_data.py::ImagePatchProcessor.target_grid` | **同一个 V4.1 模型出现 CPT 和 SFT 两套 resize 规则。** 新子类唯一差异是在 minimum-pixel 放大后先 `int()`，但它只修 SFT，不修共享 `ImagePatchProcessor`。 | 官方 V4.1 当前 `inference/image_processor.py::plan_image_grid` 确实先对放大后的 width/height 做 `int`，见 [official image_processor.py](https://huggingface.co/deepseek-ai/DeepSeek-V4.1-Flash/blob/main/inference/image_processor.py)。这不是 SFT 特性，而是**共享图像协议**应统一。实测 `(h,w)=(100,101)` 时，共享实现返回 `(39,40)`，SFT 子类返回 `(39,39)`；扫描 `(h,w)` 在 `1..199` 范围内有 `5,284+` 组结果不同，在 `100..543` 范围内有 `27,316` 组不同。根因是共享版本放大后保留 float，随后 `_safe_resize` 走 `ceil`；SFT 子类先 `int` 截断，再进入 `_safe_resize`，边界上可少一个 patch。现有 `test_target_grid_fixed_oracle` 的 5 组尺寸恰好全部未命中该 rounding 分歧。 | **删除 `DeepSeekV41SFTImagePatchProcessor`。** 如果官方语义应以当前 V4.1 为准，就直接修共享 `ImagePatchProcessor.target_grid`，让 CPT/SFT 共用；把 `(100,101)` 或其它明确命中差异的尺寸加入 `test_target_grid_fixed_oracle` 作为 rounding-sensitive 固定 oracle，避免再分叉。 |
...
| R11 | **Positive / 架构归位** | `torchtitan_npu/models/deepseek_v4_1/vision_language_dataset.py`、`vision_language_encoder.py`；`torchtitan_npu/override/deepseek_v4_1/vision_language_dataloader.py` | **目录归属本身正确，应明确保留。** V4.1 专属的数据 schema/encoding 能力放在 `models/deepseek_v4_1/`，用户可选择的 dataloader 替换通过 `override/deepseek_v4_1/` 的显式 `@override` 暴露，没有放进 `patches/`。 | 这与仓库 `.agents/AGENTS.md` 的职责划分一致：模型限定实现归 `models/<model>/`，配置级组件替换归 `override/`，而 `patches/` 仅用于上游临时补丁或运行时兼容。R1-R3 对“入口数量、重复生命周期、第二份 encoding 语义实现”的批评不等于目录放置错误。 | **保持当前目录归属。** 后续按 R1-R3 做删减/解耦时仍在这两个职责域内收敛，不要把 SFT 模型逻辑挪到 `patches/`，也不需要为它新建 Extension 层。 |
| R12 | **Info / loss 与监控口径** | 固定上游 TorchTitan v0.3.0：`torchtitan/trainer.py:684-686, 884-903, 959-988`；`torchtitan/components/loss.py:60-65, 338-367`；`torchtitan/components/metrics.py:464-501` | **SFT 大量 `-100` label 对训练 loss 归一化没有 correctness 问题；但 `n_tokens_seen` / TPS 统计的是处理过的序列 token，不是监督 token。** | CE 使用 `reduction="sum"` 且 `ignore_index=-100`；Trainer 在每个 optimizer step 先以 `(labels != IGNORE_INDEX).sum()` 得到 `local_valid_tokens`，跨 DP 求和为 `global_valid_tokens`，随后 `CrossEntropyLoss` 用该值归一化，`global_avg_loss` 也是全局 loss sum / 全局有效监督 token。因此不会按 `seq_len` 或全部 padding/prompt token 归一，PR 描述中的 `1.10/1.97` 不会因 assistant-only mask 而出现系统性口径偏差。TorchTitan v0.3.0 默认 metrics 未计算/记录 PPL；若外部用 `exp(global_avg_loss)` 派生 PPL，其口径也是监督 token 平均 CE。另一方面，数据迭代与 `ntokens_seen` 都用 `labels.numel()`，所以 `throughput(tps)` 和 `n_tokens_seen` 会包含 prompt、图片占位对应位置和 padding/ignore 位置；这是 processed-token throughput 而非 target-token throughput。 | **无需修改 loss 路径。** 保持上游现有 `global_avg_loss` 口径；文档/监控说明 TPS 与 `n_tokens_seen` 是处理 token 数。若 SFT 产品确实需要“监督 token 进度/吞吐”，新增独立 `supervised_tokens_seen` / `supervised_tps` 指标，直接复用 `local/global_valid_tokens`，不要改写既有 TPS / `n_tokens_seen` 的语义。 |
...
| 位置 | 结论 |
| --- | --- |
| `torchtitan_npu/models/deepseek_v4_1/vision_language_*.py` + `torchtitan_npu/override/deepseek_v4_1/vision_language_dataloader.py` | 见 R11：模型限定能力放 `models/<model>/`，配置级切换使用显式 `@override(target=DeepSeekV41DataLoader.Config, fqns=["dataloader"], exact=True)`，不进入 `patches/`；目录职责正确，应保留。 |
...
| V4.1 resize 官方顺序 | shared processor 与官方 `plan_image_grid` 同语义 | 现有 `test_target_grid_fixed_oracle` 的 5 组样例没有覆盖 int-rounding 差异；实测 `(100,101)` 可稳定区分 `(39,40)` 与 `(39,39)` | 增加至少一个 rounding-sensitive 固定尺寸 oracle | **部分覆盖** |
...
| tokenizer asset 升级兼容性 | 当前作者资产运行成功 | R2 的第二份 encoding 语义实现需要架构消除；单次 ST 不能证明未来 encoding 版本兼容 |
...
| 4 | 重做 encoder seam：不复刻官方完整 preprocessing/render 语义；推进/使用稳定 assistant-span API，并把 tagged image 解析职责收敛到单层 | 消除最易随 tokenizer asset 升级 break 的第二份实现与双解析 |
...
**最终 Maintainer 意见：** SFT 功能方向可以接受，目录归位与 override 切换方式也正确（R11）；TorchTitan v0.3.0 对 `-100` supervision 的 loss 归一化同样没有问题（R12）。但当前实现的新增复杂度明显高于真实产品语义所需。合入前应先完成 R1-R5 的架构收敛，尤其删除额外 shell、统一 image resize、复用既有 data lifecycle、解除对官方 encoding 内部语义的第二份实现并收敛 tagged image 解析职责，再基于收敛后的代码补最小 UT/ST 与文档。当前版本不建议直接合入。
