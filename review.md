...
| R12 | **Major / Tests** | `tests/unit_tests/override/common/test_moe_score_absorption.py::test_standard_dispatchers_reuse_torchtitan_helpers` | 测试从“helper identity”改成一次随机 `patched vs upstream` 比较。它没有固定 local generator；oracle 只是另一实现，而不是显式 stable-order 期望；只直接调用 Local override，没有证明这个 workaround 在真实 compile/spmd_types 消费路径上成立。 | 发生重复 expert-id、边界 expert-id、N=0/小 N 或 compile fake-eval 回归时，当前测试的证明力不足。 | 若 R6 后仍保留 workaround：使用确定性 literal 构造重复 expert ids，直接断言 expected permutation、score 对齐和 routed input；覆盖 Local/AllToAll 的实际消费入口，并增加真实 `torch.compile(..., backend="aot_eager", fullgraph=True)` CPU/NPU 最小验证。不要用未固定的 `torch.randn/torch.rand` 来证明稳定排序语义。 |
...
| stable argsort workaround 保持稳定顺序 | 明确 expected permutation，覆盖重复 expert ids，并进入真实 consumer/compile | 当前仅一次随机 patched-vs-upstream Local helper 比较 | **部分覆盖，oracle 弱** |
...
| 文件 | Review 结果 |
|---|---|
| `examples/deepseek_v41/debug/deepseek_v41_flash_8p_cpt_4k.sh` | rename 本身可接受；但当前仍显式 `--compile.no-enable`，与 PR 描述的 `COMPILE_BACKEND=...` 复现方式冲突。建议依赖默认关闭并让用户只用 CLI 打开。 |
| `examples/deepseek_v41/readme.md` | **需修改**：aot_eager/inductor 支持状态写错且同句自相矛盾；补真实 CLI compile 示例。 |
| `tests/unit_tests/models/deepseek_v41/test_independence.py` | 路径随 rename 更新正确，无额外问题；但它不是 compile 功能测试。 |
| `tests/unit_tests/override/common/test_moe_score_absorption.py` | **需修改**：新增 stable-sort 测试证明力不足，且它被迫为一个本不应混入 #4095 patch 的行为改测试。 |
...
| `torchtitan_npu/models/deepseek_v41/sparse_attention.py` | vectorized formulation 是 compile 所需的核心新增，但必须补 forward/backward equivalence UT；修 chunk 内重复 `[S,S]` mask 和全局 env/state；删 `cont_valid`。 |
...
| `sparse_attention.py` module docstring | 仍把实现描述为单一 eager per-document core | 更新为 eager reference + compile vectorized 双路径（在重构后按最终形态写） |
