...
| `dsv4_smla_1rank_inductor_rope` | DeepSeek-V4 | 1 Rank + `decomposed` RoPE | 1 | `inductor` | 否 | Inductor compile/execute smoke；上板时人工检查 replacement statistics 确认 pre-AOT patterns 生效 |
...
两个 DeepSeek-V3.2 case 同样设置 `check_loss=True`，使用 `decomposed` RoPE override、Ascend DSA
